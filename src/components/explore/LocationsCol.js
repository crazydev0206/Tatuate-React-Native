...
export default class LocationsCol extends React.Component{
    goToArchive(term){
        const {id} = term;
        if( null != id ){
            filterByLoc( id )
        ...
        if( null != data && Array.isArray(data) && data.length > 0 ){
  		...
  		return(
  			<View style={styles.container}>
                <SectionTitle title={title} showAll={show_view_all} allText={viewall_text} onViewAllPress={ ()=>NavigationService.navigate('Locations') }/>
